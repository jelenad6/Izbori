...
public class ReplicaNode extends ReplicatedLogServiceImplBase implements Runnable, ReplicatedLog.LogReplicator{
		...
		public void replicateOnFollowers(Long entryAtIndex, byte[] data) {
			...
			for (FollowerGRPCChannel grpcChannel : followersChannelMap.values()) { //za sync, dodat debug

			   /* System.out.println(
			            "Replicating log #" + entryAtIndex
			                    + " to follower "
			                    + grpcChannel.getConnectionString()
			    );*/
			    ...
			    if (response.getStatus() == LogStatus.LOG_HASNT_LAST_ENTRY) {
			        long followerLastIndex = response.getLastEntryIndex();
			        sendMissingEntries(grpcChannel, followerLastIndex + 1);
			    }
			}
			
		}
		
		private void sendMissingEntries( // za sync
		        FollowerGRPCChannel grpcChannel,
		        long startIndex
		) {
		    try {
		        var missingEntries =
		                replicatedLog.getLogEntriesFrom(startIndex);

		        long currentIndex = startIndex;

		        for (String entry : missingEntries) {

		            LogEntry missingLogEntry =
		                    LogEntry.newBuilder()
		                            .setEntryAtIndex(currentIndex)
		                            .setLogEntryData(
		                                    ByteString.copyFrom(entry.getBytes())
		                            )
		                            .build();
		            ...
		            grpcChannel.getBlockingStub().appendLog(missingLogEntry);
